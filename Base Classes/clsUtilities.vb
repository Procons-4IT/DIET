...
#End Region
    ...
    Public Function removeExcludeDocumentRowfromProgram(ByVal strPMRef As String) As Boolean
        Try
            ...
            Try
                ...
                strQuery = " Select U_CardCode From [@Z_OCPM] Where DocEntry = '" + strPMRef + "'"
                oRecordSet = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                oRecordSet.DoQuery(strQuery)
                If Not oRecordSet.EoF Then

                    Dim strCardCode As String = oRecordSet.Fields.Item(0).Value
                    Dim strCPRef As String = oApplication.Utilities.getRecordSetValueString("Select DocEntry From [@Z_OCPR] Where U_CardCode = '" & strCardCode & "'", "DocEntry")

                    Dim strExDate As String = String.Empty
                    oGeneralParams.SetProperty("DocEntry", strCPRef)
                    oGeneralData = oGeneralService.GetByParams(oGeneralParams)

                    strQuery = " Select Convert(VarChar(8),U_ExDate,112) As 'U_ExDate', VisOrder From [@Z_CPR4] Where DocEntry = '" & strCPRef & "'"
                    strQuery += " And Convert(VarChar(8),U_ExDate,112) > (Select Top 1  Convert(VarChar(8),U_PToDate,112) From [@Z_OCPM] Where U_CardCode = '" + strCardCode + "' And U_DocStatus In ('O','C') Order By  U_PToDate Desc )"
                    strQuery += " Order By VisOrder Desc "
                    oRecordSet.DoQuery(strQuery)

                    Dim dtCurrDate As Date = CDate(System.DateTime.Now.Year.ToString() + "-" + System.DateTime.Now.Month.ToString() + "-" + System.DateTime.Now.Day.ToString())
                    If Not oRecordSet.EoF Then
                        While Not oRecordSet.EoF
                            strExDate = oRecordSet.Fields.Item("U_ExDate").Value
                            Dim dtFromDate As Date = CDate(strExDate.Substring(0, 4) + "-" + strExDate.Substring(4, 2) + "-" + strExDate.Substring(6, 2))
                            If dtFromDate > dtCurrDate Then
                                oGeneralDataCollection = oGeneralData.Child("Z_CPR4")
                                oGeneralDataCollection.Remove(oRecordSet.Fields.Item("VisOrder").Value - 1)
                            End If
                            oRecordSet.MoveNext()
                        End While
                        oGeneralService.Update(oGeneralData)
    ...
    Public Function removeExcludeDocumentRow(ByVal strCardCode As String, ByVal strCPRef As String) As Boolean
        Try
